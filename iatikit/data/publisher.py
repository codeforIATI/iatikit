...
class Publisher(object):
    ...
    def show(self):
        ...
        if name:
            ...
            return True
        logging.getLogger(__name__).warning("Can't show publisher - metadata missing.")
        return False

    @property
    def datasets(self):
        """Return an iterator of all datasets for this publisher."""
        data_path = join(self.data_path, "*") if self.data_path else None
        metadata_path = join(self.metadata_path, "*") if self.metadata_path else None
...
class PublisherSet(GenericSet):
    ...
    def __iter__(self):
        data_paths = {
            basename(x): x for x in glob(self.data_path) if not x.endswith(".json")
        }
        metadata_paths = {
            basename(x): x for x in glob(self.metadata_path) if not x.endswith(".json")
        }
        metadata_filepaths = {
            splitext(basename(x))[0]: x for x in glob(self.metadata_path + ".json")
        }
        paths = {
            x: (data_paths.get(x), metadata_paths.get(x), metadata_filepaths.get(x))
            for x in set(

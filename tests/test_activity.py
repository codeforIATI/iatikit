...
from iatikit.data.activity import Activity, ActivitySet
from iatikit.data.dataset import Dataset, DatasetSet
...
class TestActivitySet(TestCase):
    ...
    def test_activities_filter_by_description(self):
        ...
        acts = self.fixture_org_acts.where(description=description).all()
        assert len(acts) == 1
        assert acts[0].iati_identifier == "GB-COH-01234567-Humanitarian Aid-1"
    ...
    def test_activities_filter_by_sector_exists(self):
        acts = self.fixture_org_acts.where(sector__exists=False).all()
        assert len(acts) == 1

    def test_activities_filter_by_planned_start_v2(self):
        acts = self.fixture_org_acts.where(planned_start="2011-11-01").all()
        assert len(acts) == 1
        assert acts[0].iati_identifier == "GB-COH-01234567-1"

    def test_activities_filter_by_actual_start_v2(self):
        acts = self.fixture_org_acts.where(actual_start="2011-10-01").all()
        assert len(acts) == 1
        assert acts[0].iati_identifier == "GB-COH-01234567-1"

    def test_activities_filter_by_planned_end_v2(self):
        planned_end = datetime.date(2019, 9, 30)
        acts = self.fixture_org_acts.where(planned_end=planned_end).all()
        assert len(acts) == 1
        assert acts[0].iati_identifier == "GB-COH-01234567-1"

    def test_activities_filter_by_actual_end_exists(self):
        acts = self.fixture_org_acts.where(actual_end__exists=True).all()
        assert len(acts) == 1
        assert acts[0].iati_identifier == "GB-COH-01234567-Humanitarian Aid-1"

    def test_activities_filter_by_xpath(self):
        acts = self.fixture_org_acts.where(xpath='activity-status/@code="2"').all()
        assert len(acts) == 1
        assert acts[0].iati_identifier == "GB-COH-01234567-Humanitarian Aid-0"

    def test_activities_filter_by_humanitarian(self):
        acts = self.fixture_org_acts.where(humanitarian=True).all()
        assert len(acts) == 1
        assert acts[0].iati_identifier == "GB-COH-01234567-1"

    def test_activities_filter_by_not_humanitarian(self):
        acts = self.fixture_org_acts.where(humanitarian=False).all()
        assert len(acts) == 3
...
class TestActivity(TestCase):
    ...
    def test_activity_show(self, fake_open_new_tab):
        url = "http://d-portal.org/q.html" + "?aid=GB-COH-01234567-Humanitarian+Aid-0"
        self.activity.show()
